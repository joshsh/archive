...
intfield *newIntField(int length, int height, int val, int wraparound){
  ...
  wrigley->first=linerunner2;
  wrigley->first->first=pixrunner2;
  linerunner=wrigley->first;
  ...
  for (i=0; i<height; i++){
    ...
    if (i == height-1){
      ...
      else linerunner->next=wrigley->first;
    }else{
...
intnode *at(intfield *ofdreams, int i, int j){
  ...
  if (i == ofdreams->i){
    ...
    if (i >= ofdreams->i){
      linerunner=ofdreams->ati;
      for (k=0; k < i-ofdreams->i; k++) linerunner=linerunner->next;
      pixrunner=linerunner->first;
      for (k=0; k<j; k++) pixrunner=pixrunner->next;
      ofdreams->i=i;
      ofdreams->j=j;
  ...
  for (i=0; i<height; i++){
    for (j=0; j<length; j++){
      ...
      for (k=0; k<numcolors; k++){
        if ((red == prunner->red)&&(green == prunner->green)&&(blue == prunner->blue)){ found = 1; k=numcolors; };
      ...
      if (!found){
        if (numcolors > 0){
          ...
          //prunner=sitter;
          //for (k=1; k < numcolors; k++) prunner=prunner->next;
          prunner2=new pixel256;
          prunner2->red=red;
          prunner2->green=green;
          prunner2->blue=blue;
          prunner2->next=NULL;
          prunner->next = prunner2;
        }else{
          sitter->red = red;
          sitter->green = green;
          sitter->blue = blue;
...
int findNetscapeColor(int goodcolor){
  int activated;
  for (int ii=1; ii<6; ii++){
    if (goodcolor < netlist[ii]) return netlist[ii-1];
  };
  return netlist[5];
}

void netscapeizeImage256(image256 *leander){
  leander->updated=0;
  ...
  for (i=0; i<height; i++){
    for (j=0; j<length; j++){
      pixrunner->red=findNetscapeColor(pixrunner->red);
      pixrunner->green=findNetscapeColor(pixrunner->green);
      pixrunner->blue=findNetscapeColor(pixrunner->blue);
      if (j < length-1) pixrunner=pixrunner->next;
    };
    if (i < height-1){
      linerunner=linerunner->next;
...
image256 *blake_to_image256(blake *alding){
  image256 *delvage = newImage256(alding->length, alding->height, 0, 0, 0, alding->wraparound);
...
blake *image256_to_blake(image256 *alding){
  blake *delvage = newBlake(alding->length, alding->height, 0, alding->wraparound);
