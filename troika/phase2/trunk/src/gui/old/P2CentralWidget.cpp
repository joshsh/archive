...
*/
    ...
    ToggleWidget *binder = new ToggleWidget( this, "test binder" );
    binder->move( 5, 20 );
    ...
    //ohio.setGeometry( 0, 0, 30, 30 );
    //ohio.setFont( QFont( "Times", 18, QFont::Bold ) );
    //ohio.setText( QString("foobar") );

    // QTable test.
    //*
    Q3Table *table = new Q3Table( 3, 3, this );
    // Disallow the ability to select table cells.
    table->setSelectionMode( Q3Table::NoSelection );
    table->setPixmap( 0, 0, ohio_pix );
    table->setPixmap( 1, 0, ohio_pix );
    table->setText( 1, 1, "Some text." );
    table->move( 50, 50 );
    //*/
}
...
painter.drawImage( 5, 140,  // dest. origin
    ...
    //~
    #ifndef ARM_COMPILE
    Q3UrlOperator op( "ftp://ftp.is.co.za/rfc/" );
    // do some other stuff like op.listChildren() or op.mkdir( "new Dir" )
    const Q3NetworkOperation *no = op.get( "rfc1808.txt" );
