...
SEQUENCE *decode_parser_expression(DOM_Element *el)
{
...
SEQUENCE *resolve_nonlocal_ids(SEQUENCE *term)
{
	SEQUENCE *newseq = term;

    if ((P2_type) term->type == P2_id_type)
    {
        P2_identifier *id = (P2_identifier *) term->value;
        if (id->dataset_id)
        {
...
//cout << "+ P2_evaluate_expression" << endl; cout.flush();
    ...
    SEQUENCE *term = decode_parser_expression(&document);
    ...
    {
        // Resolve identifiers.
        term = sequence__replace_atoms(term, resolve_nonlocal_ids);
        //sequence__forall_atoms(term, resolve_nonlocal_ids);
        term = sequence__normalize_la(term);

        char *local_id = NULL;

        SEQUENCE *id_seq = NULL;

        // Submit to the approprite data set.
        if (name != NULL)
        {
            id_seq = P2_parse_identifier(name);
            ...
            {
                local_id = ((P2_identifier *) (id_seq->value))->local_id;
